...
此外、您也可以按一下 image:button_down_caret.png["減少需求"] 可讓任何群組查看屬於群組的使用者清單。
...
如果您想要在調整頁面內容之後、將其下載為報告、請按一下 image:button_download.png["下載按鈕"] 用於儲存.CSV檔案的按鈕。

image:screenshot_compliance_investigation_filtered.png["在調查頁面中調整結果時可用篩選器的快照。"]

* 最上層索引標籤可讓您檢視檔案（非結構化資料）、目錄（非結構化資料）或資料庫（結構化資料）的資料。
